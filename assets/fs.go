...
// FS returns a new FS that wraps the given directory and embedded FS.
// the embed.FS is expected to embed the same files as the directory FS.
func FS(embed fs.FS, dir string) assetsFS {
...
// ErrNotExist.
func (f assetsFS) Open(name string) (file fs.File, err error) {
